...
class UserProfile(models.Model):
    ...
    check_date = models.DateField('check_date', help_text="上次签到日期", blank=True, null=True, auto_now=True)
    ...
    favourite = models.ManyToManyField(Article, through="favourites", help_text="我的收藏",
                                       related_name="user_favour")  # 生词表
...
class CommentList(models.Model):
    ...
    article = models.ForeignKey(Article, on_delete=models.CASCADE, help_text="文章")  # 单词
    content = models.TextField(help_text="评论内容")
    time = models.DateTimeField(auto_now_add=True)
    like = models.ManyToManyField(UserProfile, through="like", help_text="点赞",related_name="user_like")
...
class Like(models.Model):
    userprofile = models.ForeignKey(UserProfile, on_delete=models.CASCADE, help_text="用户")  # 和用户绑定
    comment = models.ForeignKey(CommentList, on_delete=models.CASCADE, help_text="评论", related_name="like_comment")  # 单词

    class Meta:
        unique_together = ('userprofile', 'comment')  # 唯一性
